...
for z in range (0, len(thick)):   
    ...
    xlabel(r'$\rm Wavelength \ (nm)$', fontsize = '30')

    #plt.plot(k, R_exp, label = "grabbed data %d nm" %thick[z])
    plt.plot((1e4)/k, Rlow, linewidth = 2, color = shades[z], label = r"$\rm \ hBN \ thicknes \ is \ %d \ nm \ thick$" %thick[z])
...
plt.savefig("hBNAllThickBaF2Normal_Lam.png")
plt.savefig("hBNAllThickBaF2Normal_Lam.pdf")
plt.show()
