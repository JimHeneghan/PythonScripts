...
ax0.set_xticks([])
ax0.set_yticks([])
ax0.set_ylabel('Absorptance', rotation='vertical', fontsize = '70', labelpad = 50)
...
for i in range (0, 3):
	# base = 2 + 4*i
	namy = "RTADataBareDevice/2_%.02dumPitchRTA.txt" %base[i]
	lam, A = loadtxt(namy, usecols=(0,3), skiprows= 1, unpack =True)
	print(namy)
	lam = 1/(lam*1e-4)
	ax1.plot(lam, A,  color = ThreeColor[i], linewidth = 3)
...
for i in range (0, 3):
	# base = 2 + 4*i
	namy = "RTADataDeadDevice/2_%.02dumPitchRTA.txt" %base[i]
	print(namy)
	lam, A = loadtxt(namy, usecols=(0,3), skiprows= 1, unpack =True)
	lam = 1/(lam*1e-4)
	ax2.plot(lam, A,  color = ThreeColor[i], linewidth = 3)
...
for i in range (0, 3):
	# base = 2 + 4*i
	namy = "RTADataFullDevice/2_%.02dumPitchRTA.txt" %base[i]
	lam, A = loadtxt(namy, usecols=(0,3), skiprows= 1, unpack =True)
	lam = 1/(lam*1e-4)
	ax3.plot(lam, A,  color = ThreeColor[i], linewidth = 3)
...
namy = "hBN_VacRTA.txt" 
lam, R, T, Ah = loadtxt(namy, usecols=(0,1,2,3), skiprows= 1, unpack =True)
...
ax3.plot(1/(lam*1e-4), A+Ah, label = r'$ A_{hBN \ 80 \ nm}$', color = "gold", linewidth = 4)
...
dcc = [2.10, 2.38, 2.58]
for i in range(0, 3):
    temp = mpatches.Patch(facecolor=ThreeColor[i], label = r'$ d_{cc} \rm = %2.2f \ \mu m$' %dcc[i], edgecolor='black')
    patches.append(temp) 
...
plt.savefig("Fig_3_3line.png")
plt.savefig("Fig_3_3line.pdf")
