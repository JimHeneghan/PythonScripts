...
for z in range(0, len(freq)):
	...
	# plt.xlim(0.1, 2)
	#print X[0:15]
	ax.plot(X[8:121] - 1.15, Full[110][8:121] + 2.2*(z), linewidth=2, color = cmatch[len(cmatch)-z - 1], label = r"$ \rm %s \ (\mu m)$" %freq[z])
...
plt.savefig("EFieldXSec_Line_Ex_.png")
plt.savefig("EFieldXSec_Line_Ex_.pdf")
# plt.show()
