...
ax3.loglog(thick, 2*math.pi*c0*y, "ro", markersize=10, markeredgecolor = "black", markerfacecolor = 'red', label = r"$\rm Calculated \ \gamma_{x,y}$")# shades[z])#, label = r"$\rm Calculated \ \gamma: \ d = %d $" %thick[z], zorder = 5)

ax3.scatter(83, 2*math.pi*c0*(exp(3.46532237043)*100), s = 100, facecolors = "none", edgecolor = "limegreen",linewidth=3.0, zorder = 6, label = r"$\gamma_{x,y} \rm = %5.3f \times 10^{12} \ (rad \ s^{-1}) $" %(2*math.pi*c0*(exp(3.46532237043)*100/1e12)))
...
print min(RT[0:1000])
wn = (1/(Lambda))/100
ax2.plot(wn,Ref, label = r'$\rm R_{FDTD}$', color = "red", linewidth = 3)
ax2.plot(wn,Trans, label = r'$\rm T_{FDTD}$', color = "black", linewidth = 3)
ax2.plot(wn,(1-RT), label = r'$\rm A_{FDTD}$', color = "limegreen", linewidth = 3)
...
plt.tight_layout()
plt.savefig("AllTogether_JimCode_Working.png")
plt.savefig("AllTogether_JimCode_Working.pdf")
# plt.show()
