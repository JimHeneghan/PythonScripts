...
ExR, ExI, EyR, EyI, EzR, EzI, = np.loadtxt("SensorTry2.txt",  usecols=(5,6,7,8,9,10), skiprows= 1, unpack =True)
freq, ExIncR, ExIncI, EyIncR, EyIncI, EzIncR, EzIncI = np.loadtxt("SourceTry2.txt",  usecols=(0,1,2,3,4,5,6), skiprows= 1, unpack =True)

for i in range (0, Nx):
	for j in range (0, Ny):
		for f in range (0, NFREQs):
			ExInc[f,i,j] = (ExIncR[f] + 1j*ExIncI[f])
			EyInc[f,i,j] = (EyIncR[f] + 1j*EyIncI[f])
			EzInc[f,i,j] = (EzIncR[f] + 1j*EzIncI[f])
