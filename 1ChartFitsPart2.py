...
for z in range (4, 7):   
    ...
    axs[q].plot(k_data, func(k_data, *popt), 'g--', linewidth = 3, label = r"$\rm \gamma_{fit} = %.2f$ " %popt[0])# "\n" r"J = %f" %tuple(popt))
    axs[q].plot(k_data, R_exp, linewidth = 3)#, label = "Digitized Data")
    plt.setp(axs[q].spines.values(), linewidth=2)
    axs[q].set_ylabel("R", fontsize = '30')
    axs[q].set_ylim(0,1)
    axs[q].set_title("d = %d nm" %thick[z], fontsize = '25')
    axs[q].tick_params(width=2, labelsize=20)
    #box = ax.get_position()
    ...
    #else:
    axs[q].legend(fancybox = True, loc='upper left', fontsize='17', framealpha=1)
    #if(m == 1):
        
    if (q == 2):
        axs[q].set_xlabel(r'$\rm Frequency \ (cm^{-1})$', fontsize = '30')
    q  = q + 1
    # if (m == 1):    
    #     q = 1
    #     m = 0
...
plt.savefig("/halhome/jimheneghan/UsefulImages/hBNThicknessGammaFiPart2.png")
plt.savefig("/halhome/jimheneghan/UsefulImages/hBNThicknessGammaFitPart2.pdf")
    #plt.cla()
plt.show()
