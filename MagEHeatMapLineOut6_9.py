...
for z in range(3, 4):
	...
	ax.plot(X, Full[103], linewidth=2, color = shades[len(shades)-z - 1], label = r"$ \rm %s \ \mu m$" %freq[z])
...
X = X - max(X)/2
ax1.plot(X*1e6, E, color = 'black', linewidth = 4)

ax1.set_ylabel(r"$ \rm Intensity $", fontsize = '20')
