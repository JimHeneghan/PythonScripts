...
Field = "6.5um.txt"
Full = np.zeros((363, 211), dtype = np.double)

X = np.linspace(0,  2.31, 211)
Y = np.linspace(0,  4.01, 363)
for i in range (0, 359):
	E = np.loadtxt(Field, usecols=(i,), skiprows=581, unpack =True )
	Full[i] = E 
...
plt.tick_params(left = False, bottom = False)
# plt.xlim(0.1, 2)
plt.plot(X[5:205] - 1.15, Full[200][5:205], color = "black", linewidth=2)
ax1.axvline(x = -0.68, linestyle = "dashed", color = 'black')
ax1.axvline(x =  0.68, linestyle = "dashed", color = 'black')
plt.savefig("6.5umSourceXSecLineMar15.png")
plt.savefig("6.5umSourceXSecLineMar15.pdf")
plt.show()
