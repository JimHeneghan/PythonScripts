...
fig, (ax1, ax3, ax2) = plt.subplots(3, figsize = (18,30))
...
ax3.scatter(80, 2*math.pi*c0*(exp(3.484986312)*100), s=600, facecolors = "none", edgecolor = "limegreen",linewidth=8.0, zorder = 6, label = r"$\gamma_{x,y} \mathrm{ \ (80 \ nm) = %5.3f \times 10^{12} \ (rad \ s^{-1})} $" %(2*math.pi*c0*(exp(3.484986312)*100/1e12)))

ax3.legend(fancybox = True, loc='lower center', fontsize='30', framealpha=0.2)
...
ax2.tick_params(direction = 'in', width=2, labelsize=30)
ax2.set_ylabel(r"$R, T, A$", fontsize = '70')   
...
ax2.plot((k0*1e-2), Rm, label=r'$R_{TM}$', color='darkred', linewidth = 18)
ax2.plot((k0*1e-2), Tm, label=r'$T_{TM}$', color='blue', linewidth = 18)
ax2.plot((k0*1e-2), (1-(Rm + Tm)),  label=r'$A_{TM}$', color='darkgreen', linewidth = 18)

ax2.plot(wn,Ref, label = r'$R_{FDTD}$', color = "pink", linewidth = 7)
ax2.plot(wn,Trans, label = r'$T_{FDTD}$', color = "cyan", linewidth = 7)
ax2.plot(wn,(1-RT), label = r'$A_{FDTD}$', color = "lime", linewidth = 7)

ax2.legend(loc='center right', fontsize='30')
ax2.axvline(x = 1/(7.26*1e-4), color = 'black', linewidth = 2)
ax2.set_yticks(np.arange(0.1, 1, 0.2))
plt.tight_layout()
plt.savefig("AllTogether_JimCode_Working_Dec22.png")
plt.savefig("AllTogether_JimCode_Working_Dec22.pdf")
# plt.show()
