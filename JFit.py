...
for z in range (0, len(thick)):   
    ...
    xlabel(r'$\rm Wavelength \ (cm^{-1})$', fontsize = '30')
    ...
    plt.plot(k_data, func(k_data, *popt), 'g--', label = r"$\rm \gamma_{fit} = %g$ "" \n hBN Thickness = %g nm" %(popt, thick[z]))
    plt.plot(k_data, R_exp)
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    if(z < 3):
        ax.legend(fancybox = True, bbox_to_anchor=(1.01, 1), loc='upper right', fontsize='22', framealpha=1)
    else:
        ax.legend(fancybox = True, loc='upper left', fontsize='22', framealpha=1)
