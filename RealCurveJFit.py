...
for z in range (0, len(thick)):   
    ...
    plt.plot(k_data, func(k_data, *popt), 'g--', linewidth = 3, label = r"$\rm \gamma_{fit} = %f$ " "\n" r"J = %f" %tuple(popt))
    ...
    if(z < 3):
        ...
        ax.legend(fancybox = True, loc='upper left', fontsize='25', framealpha=1)
    #    legend(loc='lower center', fontsize='30')
    plt.tight_layout()
    plt.setp(ax.spines.values(), linewidth=2)
    plt.savefig("JCurveFitConstrain/hBNCurveFit%gnm.png" %thick[z])
    plt.savefig("JCurveFitConstrain/hBNCurveFit%gnm.pdf" %thick[z])
    plt.cla()
    #plt.show()
