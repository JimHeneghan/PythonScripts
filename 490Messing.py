...
for z in range (3,4):   
    ...
    popt, pcov = curve_fit(func, k_data, R_exp, bounds = (0, 1))
    print popt
    ...
    axs.plot(k_data, func(k_data, *popt), 'g--', linewidth = 3, label = r"$\rm \gamma_{fit} = %.2f$ " %gamma)# "\n" r"J = %f" %tuple(popt))
    print"thickness = %d \t gamma = %0.2f \t J = %0.2f " %(thick[z], gamma, popt[0])
