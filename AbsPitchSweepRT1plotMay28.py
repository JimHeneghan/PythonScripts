...
#xlim(6,7)
#ylim(0.245, 0.275)
numbers = numbers = [2.24, 2.29, 2.3, 2.36, 2.42, 2.48, 2.51, 2.56, 2.6]
Peak = []
for i in range (0,9):
    E1 = "%s" %numbers[i]
    E2 = "PitchRT.txt"
    ...
    if(i == 2):
        plt.plot(Lambda, (1-RT), linewidth = 5, color = "red", label = r"$\rm d_{CC} = 2.3  \ \mu m$")
        # plt.plot(Lambda, -1*T, linewidth = 5, color = "limegreen", label = r"$\rm pitch = 2.%d \ \mu m$" %numbers[i])
    else:
        plt.plot(Lambda, (1-RT), linewidth = 2, color = "black", label = r"$\rm d_{CC} = %s \ \mu m$" %numbers[i])
...
plt.savefig("AgPatternPitchSweepAbs.pdf")
plt.savefig("AgPatternPitchSweepAbs.png")
