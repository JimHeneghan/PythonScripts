...
lambda1, n_r, n_i = loadtxt("hBNgetfdtdindex.txt", usecols=(0,1,2),  unpack = True)
...
T1=abs(1.0/(cos(k*d)-1j*(nc+1.0/nc)*sin(k*d)/2.0))**2

# Load Complex Dielectric Function Data
lambda3, n_r, n_i = loadtxt("hBNgetfdtdindexMO2.txt", usecols=(0,1,2),  unpack = True)
...
# Calculate Complex Propagation Constant
k0 = 2.0*pi/(lambda3*1.0e-6)
k=k0*nc

# Calculate (Power) Transmision from the result of problem 5.11 
# from: http://eceweb1.rutgers.edu/~orfanidi/ewa/ch05.pdf
# Note j --> -i Convention in formula below
T3=abs(1.0/(cos(k*d)-1j*(nc+1.0/nc)*sin(k*d)/2.0))**2
...
plot(lambda2, TFDTD,  label=r'$T_{FDTD: getfdtdindex}$', color='blue' )
