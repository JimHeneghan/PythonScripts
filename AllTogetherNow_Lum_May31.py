...
ax3.set_ylabel(r'$\rm \gamma_{x,y} \ (rad \ s^{-1})$', fontsize = '15')
ax3.set_xlabel(r'$ d\rm_{hBN}$', fontsize = '15')
...
ax3.loglog(thick, 2*math.pi*c0*y, "ro", markersize=10, markeredgecolor = "black", markerfacecolor = 'red', label = r"$\mathrm{Calculated \ \gamma}_{x,y}$")# shades[z])#, label = r"$\rm Calculated \ \gamma: \ d = %d $" %thick[z], zorder = 5)
ax3.loglog(x, 2*math.pi*c0*yfit(x), color = "black", linewidth=3, label = r"$\mathrm{Power-law \ fit \ to \ \gamma}_{x,y} \mathrm{ \ vs \ } d\rm_{hBN} \mathrm{\ data}$", zorder = 1)
ax3.scatter(83, 2*math.pi*c0*(exp(3.46532237043)*100), s = 100, facecolors = "none", edgecolor = "limegreen",linewidth=3.0, zorder = 6, label = r"$\mathrm{\gamma}_{x,y} \mathrm{ \ (83 \ nm) = %5.3f \times 10^{12} \ (rad \ s^{-1})} $" %(2*math.pi*c0*(exp(3.46532237043)*100/1e12)))
...
ax2.plot((k0*1e-2), Rm, label=r'$\rm R_{TM}$', color='darkred')
ax2.plot((k0*1e-2), Tm, label=r'$\rm T_{TM}$', color='blue')
ax2.plot((k0*1e-2), (1-(Rm + Tm)),  label=r'$\rm A_{TM}$', color='green')

ax2.plot(wn,R, label = r'$\rm R_{FDTD}$', color = "red", linewidth = 2)
ax2.plot(wn,T, label = r'$\rm T_{FDTD}$', color = "black", linewidth = 2)
...
plt.tight_layout()
plt.savefig("AllTogether_Lum_Working.png")
plt.savefig("AllTogether_Lum_Working.pdf")
# plt.show()
